# Level 2 data-driven register (UC Register) using locators CSV
import csv
import time
...
def run_case(row: dict, locs: Dict[str, Tuple[str, str]]) -> None:
    ...
    try:
        ...
        if expect_type == "alert":
            assert_present(driver, locs, "alert_danger", test_id, "alert missing")
            if expect_text:
                actual = find(driver, locs, "alert_danger").text
                self_assert(expect_text, actual, test_id, "alert text")
        elif expect_type == "field_error":
            assert_present(driver, locs, "field_error", test_id, "field error")
        elif expect_type == "success":
            assert_present(driver, locs, "success_heading", test_id, "success heading")
            if expect_text:
...
def assert_present(driver, locs, name, test_id, msg):
    if not is_present(driver, locs, name):
        raise AssertionError(f"{msg} ({name})")
