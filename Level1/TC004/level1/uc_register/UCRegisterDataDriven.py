...
class UCRegisterDataDriven(unittest.TestCase):
    def setUp(self):
    ...
    def test_u_c_register_data_driven(self):
        data_file = Path(__file__).resolve().parent / "data" / "UC_REGISTER.csv"
        rows = self._load_rows(data_file)
        for row in rows:
            with self.subTest(tc_id=row.get("tc_id")):
                self._run_register_flow(row)
                # keep the driver fresh per row to avoid state bleed
    ...
    def _run_register_flow(self, row):
        ...
        # populate form fields from CSV row
        ...
        driver.find_element(By.ID, "input-email").send_keys(row.get("email", ""))
        ...
        if agree_checkbox.is_selected() != agree_val:
            # label sits on top of the checkbox; click via label to avoid interception
